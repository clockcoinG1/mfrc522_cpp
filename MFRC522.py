...
class MFRC522:
	...
	def PICC_HaltA(self):
			...
			# Calculate CRC_A
			result = [0] * 2
			status, result[0], result[1] = self.CalulateCRC(buffer[:2])

			if status != self.MI_OK:
					return status

			# Send the command.
			status, _, _ = self.MFRC522_ToCard(self.PCD_TRANSCEIVE, buffer)

			if status == self.MI_NOTAGERR:
					return self.MI_OK
			if status == self.MI_OK:
					return self.MI_ERR
	...
	def MIFARE_OpenUidBackdoor(self, logErrors):
			...
			status, received, validBits = self.MFRC522_ToCard(cmd, 1, response, received, validBits, 0, False)

			if status != self.MI_OK:
					if logErrors:
							print("Card did not respond to 0x40 after HALT command. Are you sure it is a UID changeable one?")
							print("Error name: ", status)
					return False
